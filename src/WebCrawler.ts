...
const ENABLE_BTN_CLICK = enable_button_click;
...
export class Crawler {
    ...
    async takeScreenshot(page: Page, btn_idx = -1) {
        ...
        if (screenshotBuffer != null && this.currentJob?.url) {
            ...
            if (btn_idx > -1) {
                ...
                await fse.outputFile(parentDir + "_screenshot." + imageType, screenshotBuffer);
            }
            else {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (instrumentationRecords.altered) {
                    ...
                    try {
                        // await this.handdleButtonClick(page, currentJob);
                        if (!this.alwaysScreenshot) {
                            await this.takeScreenshot(page);
                        }
                ...
                if (this.containsWebAssembly && ENABLE_BTN_CLICK) {
                    await this.handleButtonClick(pageURL);
                }
            } catch (err) {
                clearTimeout(timeout);
                reject(err);
    ...
    async screenshotPageOnly(url: string) {
        try {
            ...
            await this.closePage(page);
            if (ENABLE_BTN_CLICK) {
                await this.handleButtonClick(url);
            }
        } catch (browserErr) {
            throw browserErr
        }
